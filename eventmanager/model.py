...
from .constants import Category, emoji_class_dict, class_spec_dict
...
class Event:
        ...
        if self.entrants:
            for category in Category:
                ...
                embed.add_field(
                    ...
                    value="\n".join([f"{class_spec_dict[i.category_class]['specs'][i.spec]['emoji']} {i.user.mention} - <t:{int(i.joined_at.timestamp())}:F>" for i in ent]),
                    inline=False
                )
                
        embed.add_field(
            name=f"**{len(self.entrants)}** Joined Users: ",
            value=cf.box(f"Melee: {len(joined_melee)}\t Ranged: {len(joined_ranged)}\nHealer: {len(joined_healer)}\t Tank: {len(joined_tank)}"),
            inline=False
        )
    ...
    def end(self):
        ...
        embed.description = ""
        embed._fields.insert(0, {"name": self.name, "value": cf.box(self.description), "inline": False})
...
class Entrant:
    def __init__(self, user_id: int, event: Event, category: Category, category_class: str, spec: str, joined_at: datetime) -> None:
...
class Flags(commands.Converter):
    async def convert(self, ctx, argument: str):
        ...
        parser.add_argument("--name", "-n", type=str, help="The name of the event.", dest="name", nargs="+", required=True)
        parser.add_argument("--description", "-d", type=str, help="The description of the event.", dest="description", nargs="+", required=True)
        parser.add_argument("--description2", "-d2", type=str, help="The description of the event.", dest="description2", nargs="+", default=[])
        parser.add_argument("--end", "-e", type=str, help="The end time of the event.", dest="end", nargs="+", required=True)
        parser.add_argument("--image", "-i", type=str, help="The image url of the event.", dest="image", nargs="+", default=[])
        parser.add_argument("--channel", "-c", type=str, help="The channel to post the event in.", dest="channel", nargs="+")
        ...
        if chan:=flags.get("channel"):
            flags["channel_id"] = (await commands.TextChannelConverter().convert(ctx, " ".joint(chan))).id
