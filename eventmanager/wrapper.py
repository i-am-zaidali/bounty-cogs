...
class SoftRes:
    ...
    async def get_raid(self, token: str, raid_id: str):
        return await self._request(self.RAID_SECURE, method="POST", json={"token": token, "raidId": raid_id})
    
    async def get_gargul_data(self, token: str, raid_id: str) -> str:
        return await self._request(self.BASE_URL + f"payload/{raid_id}/gargul", method="POST", json={"token": token})
...
class SRFlags(commands.Converter):
    ...
    _parser.add_argument("-f", "--faction", choices=["Alliance", "Horde"], dest="faction", default="Horde")
    _parser.add_argument("-i", "--instance", type=str.lower, choices=["aq20", "aq40", "mc", "bwl", "onyxia", "zg", "dragonsofnightmare", "naxxramas", "kara", "magtheridon", "gruul", "doomwalker", "doomlordkazzak", "worldbosses", "gruulmag", "ssc", "tempestkeep", "ssctempestkeep", "hyjal", "blacktemple", "bthyjal", "za", "sunwellplateau",], required=True)
    ...
    _parser.add_argument("-pm", "--plus-modifier", type=int, dest="plusModifier", choices=range(1, 26))
