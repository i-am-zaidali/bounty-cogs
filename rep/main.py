...
class RepManager(commands.Cog):
    ...
    async def send_logging_embed(self, ctx: commands.Context, members: typing.List[discord.Member], amount: int, action: str, reason: str = None):
        message = (
        ...
        ) if action in [self.ADD, self.REMOVE] else (
    ...
    @is_staff()
    async def rep_add(self, ctx: commands.Context, amount: int, members: commands.Greedy[discord.Member], *, reason: str):
        """
        ...
        await ctx.maybe_send_embed(f"{cf.humanize_list([member.mention for member in members])} now have {rep} reputation.")
        
        await self.send_logging_embed(ctx, members, amount, self.ADD, reason)
        
    @rep.command(name="remove")
    @is_staff()
    async def rep_remove(self, ctx: commands.Context, amount: int, members: commands.Greedy[discord.Member], *, reason: str):
        """
        ...
        if members == failed:
            return await ctx.maybe_send_embed("All of the given members either had 0 reputation or had less rep than the amount given to remove.")
        
        await self.send_logging_embed(ctx, members, amount, self.REMOVE, reason)
        
        return await ctx.maybe_send_embed(f"Removed {amount} rep from {cf.humanize_list([member.mention for member in success])}.\nThey now have {rep} reputation.\n\n" + (f"{cf.humanize_list([member.mention for member in failed])} had 0 reputation or had less rep than the amount given to remove." if failed else ""))
        
    @rep.command(name="reset")
    @is_staff()
    async def rep_reset(self, ctx: commands.Context, members: commands.Greedy[discord.Member], *, reason: str):
        """
        ...
        if members == failed:
            return await ctx.maybe_send_embed("All of the given members already had 0 reputation so I couldn't reset any of them.")
    ...
    async def repset_showsettings(self, ctx: commands.Context):
        ...
        if staff_role is None:
            ...
            staff_role = getattr(staff_role, "mention", "Couldn't find set role. ID: {}".format(staff_role))
            
        if log_channel is None:
            log_channel = "Not set."
        else:
            log_channel = ctx.guild.get_channel(int(log_channel))
            log_channel = getattr(log_channel, "mention", "Couldn't find set channel. ID: {}".format(log_channel))
