...
class ForwardButton(PaginatorButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        self.view.previous_page = self.view.current_page
...
class PaginatorSelect(Select["Paginator"]):
    ...
    async def with_pages(cls, view: "Paginator", placeholder: str = "Select a page:"):
        ...
        if pages > 25:
            # compute a 25-item sliding window start index, centered around current_page
            # then bias the window by the delta between current_page and previous_page
            # so the window moves up/down when the user paginates.
            max_start = pages - 25
            # default center behavior: put current_page roughly in middle when possible
            if 12 < view.current_page < pages - 12:
                start = view.current_page - 12
            elif view.current_page >= pages - 12:
                start = max_start
            else:
                start = 0

            # bias sliding window by the same delta the user moved (if available)
            prev = getattr(view, "previous_page", None)
            if prev is not None:
                delta = view.current_page - prev
                start = min(max_start, max(0, start + delta))
...
class PaginatorSourceSelect(Select["Paginator"]):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await self.view.change_source(source, False)
        await self.view.edit_message(interaction)
...
class Paginator(ViewDisableOnTimeout):
    def __init__(
        ...
        self.previous_page: int = start_index
    ...
    async def update_items(self, edit: bool = False):
        ...
        for i in self.children:
            if isinstance(i, PageButton):
                ...
                self.current_page == self._start_from and isinstance(i, FirstItemButton)
    ...
    async def start(
        self, ctx: commands.Context | discord.Interaction, ephemeral: bool = True
    ):
        """
        ...
        if isinstance(ctx, commands.Context):
            ...
            if ctx.response.is_done():
                self.message = await ctx.followup.send(
