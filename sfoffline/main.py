...
class SFOffline(commands.Cog):
    ...
    async def on_typing(self, channel: discord.abc.Messageable, user: typing.Union[discord.User, discord.Member], when: datetime):
    ...
    @commands.Cog.listener()
    async def on_reaction_add(self, reaction: discord.Reaction, user: typing.Union[discord.User, discord.Member]):
        if isinstance(user, discord.Member):
            if user.bot:
                return
    ...
    @commands.Cog.listener()
    async def on_reaction_remove(self, reaction: discord.Reaction, user: typing.Union[discord.User, discord.Member]):
        if isinstance(user, discord.Member):
            if user.bot:
                return
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.status != after.status:
            if (
               after.status is self.offline_status
               or
               before.status is self.offline_status
            ):
    ...
    async def lastonline(self, ctx: commands.Context, user: discord.Member):
        ...
        if not last_online:
            ...
            statement = f"{user.mention} was last seen at {self.get_formatted_timestamps(last_online)}" 
    ...
    async def lastonline_top(self, ctx: commands.Context, x: int):
        ...
        offline_users = dict(sorted(filter(lambda x: (user:=ctx.guild.get_member(x[0])) and user.status is self.offline_status, self.cache.copy().items()), key=lambda x: x[1]))
        ...
        for page in pagify(final):
            embed = discord.Embed(title="Top {} Offline Users".format(x), description=page, color=discord.Color.red())
    ...
    async def lastonline_offline(self, ctx: commands.Context):
        ...
        if not self.cache:
            return await ctx.maybe_send_embed("I haven't tracked any users going offline or coming online yet.")
        
        final = ""
        
        sorted_dict = dict(sorted(filter(lambda x: (user:=ctx.guild.get_member(x[0])) and user.status is self.offline_status, self.cache.copy().items()), key=lambda x: x[1]))
        
        for ind, (user_id, last_seen) in enumerate(sorted_dict.items(), 1):
            final += f"{ind}. <@{user_id}> ({user_id}) - {self.get_formatted_timestamps(last_seen)}\n"
                
        for page in pagify(final):
            embed = discord.Embed(title="All Offline Users", description=page, color=discord.Color.dark_purple())
            await ctx.send(embed=embed)
